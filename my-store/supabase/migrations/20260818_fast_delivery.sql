-- PetTagi — ekspresowy czas realizacji (3–5 dni roboczych)
...
begin
  if exists (
    ...
    where table_schema = 'public' and table_name = 'orders' and column_name = 'express_fulfillment'
  ) and not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'orders' and column_name = 'fast_delivery'
  ) then
    alter table public.orders rename column express_fulfillment to fast_delivery;
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'orders' and column_name = 'express_fulfillment_cost'
  ) and not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'orders' and column_name = 'fast_delivery_cost'
  ) then
    alter table public.orders rename column express_fulfillment_cost to fast_delivery_cost;
  end if;
end $$;
...
alter table public.orders
  add column if not exists fast_delivery_cost numeric(10, 2) not null default 0;
