...
alter table public.order_items
  add column if not exists product_slug text not null default 'adresowka-klasyczna';

alter table public.order_items
  add column if not exists product_name text not null default 'Adresówka klasyczna';
...
begin
  ...
  select
    ...
    item->>'image_url',
    coalesce(nullif(item->>'product_slug', ''), 'adresowka-klasyczna'),
    coalesce(nullif(item->>'product_name', ''), 'Adresówka klasyczna'),
    item->>'ring_color',
    item->>'base_color',
    item->>'base_charms',
